...
//     } catch (err) {
//         console.error("Failed to start recording:", err);
//         setError("Could not start recording. Please grant screen sharing permissions for this tab.");
//     }
//   };
...
//   // --- FIX: Logic to prevent duplicate uploads ---
//   useEffect(() => {
//     // This effect triggers only when a recording has finished and there are chunks to upload.
//     if (!isRecording && recordedChunks.length > 0) {
      
//       // Immediately copy the chunks to a local variable and clear the state.
//       // This prevents the effect from accidentally running twice with the same data.
//       const chunksToUpload = [...recordedChunks];
//       setRecordedChunks([]);

//       const upload = async () => {
//         setUploading(true);
//         // Use the copied chunks for the upload, not the now-empty state variable.
//         const blob = new Blob(chunksToUpload, { type: 'video/webm' });
//         const duration = Math.round((Date.now() - recordingStartTime) / 1000);
...
//           setError("Failed to upload recording.");
//         } finally {
//           // Only manage the uploading state here. Chunks have already been cleared.
//           setUploading(false);
//           setRecordingStartTime(0);
//         }
//       };
